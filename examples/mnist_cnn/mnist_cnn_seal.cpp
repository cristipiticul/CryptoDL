...
int main(int argc, char *argv[]) {
    ...
    TensorP<SealCipherText> input = hetfactory.create(Shape({1, 1, 28, 28}));
    ...
    std::vector<std::vector<uint8_t>> X(
        dataset.test_images.begin(), dataset.test_images.begin() + batchSize);
    std::vector<uint8_t> Y(dataset.test_labels.begin(),
                           dataset.test_labels.begin() + batchSize);
    ...
    std::vector<SealCipherText> encImages;
    ...
    TensorP<double> decrypted =
        ((HETensor<SealCipherText> *)model.mLayers.back()->output().get())
            ->decryptDouble();
